		...
		<div class="innerOuter">
			...
			<!--
				...
				1. 기본값 : application/x-www-form-urlencoded
				 - 일반적인 텍스트 기반의 폼 데이터 전송 시 사용하는 인코딩 방식
				 - form 태그 내부의 모든 데이터를 key=value 형태로 인코딩하여 전달
				 - 단, 전달할 데이터 중 file 형태의 데이터가 포함된 경우 전송이 불가능
				2. multipart/form-data
				 - 파일 업로드시 반드시 사용해야 하는 enctype (JS 비동기 함수에서도 마찬가지)
				 - 텍스트 데이터와 바이너리 형태의 데이터(파일)를 개별적인 파트로 나누어 각각 따로 전송하는 방식
				   (web kit form boundary를 통해 영역을 구분 => 데이터를 전달하는 프레임이 바뀌는 것(char encoding이랑 다름))
			...
			id="enrollForm" method="post" enctype="multipart/form-data">
				<!-- 사용자가 입력했던 값 유지, 가독성 증가 -->
				<!-- 그 전에는 get 방식으로 똑같은 url로 요청 보냈었음. 글쓰기 버튼 -->
				<table align="center">
                    <tr>
                        <th>제목</th>
                        <td>
                        <!-- form:input 을 통해 모델어트리뷰트로 전달된 보드 객체의 boardTitle 값이 여기 바인딩 되는 것 -->
