...
public class BoardDaoImpl implements BoardDao{
	...
	public Map<String, String> getBoardTypeMap() {
		 ...
		 *  두번째 매새변수로는 어떤 칼럼을 key로 사용할지를 작성
	...
	public List<Board> selectList(PageInfo pi, Map<String, Object> paramMap) {
		 ...
		 * SELECT * 
		 * FROM(
		 * 		SELECT ROWNUM ASRNUM, INNER.*
		 * 		FROM(
		 * 		--특정 칼럼을 기준으로 조회된 쿼리 
		 * 		) INNER
		 * )
		 * WHERE RNUM BETWEEN A AND B
		 *  - 쿼리문이 복잡하고 코드의 가독성이 떨어지나 필요한 행만 조회하여 가져올 수 있기째문에
		 *    메모리 낭비나 성능저하가 별로 없다 
		 *  - 1페이지 요청시 -> WHERE RNUM BETWEEN 1 AND 10
		 *  - 2페이지 요청시 -> WHERE RNUM BETWEEN 11 AND 20
		 *  
		 * 2. RowBounds를 활용한 방식
		 *  - MyBatis에서 쿼리 결과에 대해 페이징 처리르 적용하는 도구
		 *  - 전체 쿼리결과를 자바어플리케이션으로 가져온 후, 지정한 워치(offset)에서
		 *    특정 개수(limit)를 잘라내는 방식으로 페이징 처리를 진행한다.
		 *  - 어플리케이션으로 가져올 데이터가 수만건 이상인경우 메노리 낭비 및 성능저하가 발생할 수 있다
		 *  - "소규모 데이터 쿼리" 시 사용하는것을 권장.
		 *  
		 * 3. OFFSET FETCH를 사용하여 쿼리 조회(Oracle 12c 이상만 사용가능)
		...
		int limit = pi.getBoardLimit(); // offset위치로부터 몇개의 행을 가져올지
